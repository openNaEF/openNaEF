...
import voss.nms.inventory.builder.PseudoWireCommandBuilder;
import voss.nms.inventory.builder.TextBasedPseudoWireCommandBuilder;
...
public class PseudoWireDiffUnitBuilder extends DiffUnitBuilderImpl<NetworkPseudoWire, PseudowireDto> {
    private static final Logger log = LoggerFactory.getLogger(PseudoWireDiffUnitBuilder.class);
    private Map<Port, String> portToAbsoluteNameMap;
    private DiffPolicy policy;
    private String userName;

    public PseudoWireDiffUnitBuilder(DiffSet set, Map<Port, String> portToAbsoluteNameMap,
    ...
    protected DiffUnit update(String inventoryID, NetworkPseudoWire networkPw, PseudowireDto dbPw) throws IOException,
        ...
        TextBasedPseudoWireCommandBuilder builder = new TextBasedPseudoWireCommandBuilder(dbPw, userName);
    ...
    protected DiffUnit create(String inventoryID, NetworkPseudoWire networkPw) throws IOException, InventoryException,
        ...
        long pwID;
        try {
            pwID = getPseudoWireID(networkPw);
        } catch (ExternalServiceException e) {
            throw new InventoryException(e);
        }
        List<CommandBuilder> builders = new ArrayList<CommandBuilder>();
        PseudoWireCommandBuilder builder = new PseudoWireCommandBuilder(policy.getDefaultPseudoWirePoolName(), userName);
    ...
    private void buildAc1Command(PseudoWireCommandBuilder builder, PseudoWirePort pwp) {
    ...
    private void buildAc2Command(PseudoWireCommandBuilder builder, PseudoWirePort pwp) {
        String acName = getAbsoluteNameForAc(pwp);
        if (acName == null) {
            log.warn("attachment circuit not found: " + pwp.getDevice().getDeviceName() + ":" + pwp.getPseudoWireID());
        } else {
    ...
    protected DiffUnit delete(String inventoryID, PseudowireDto dbPw) throws IOException,
            ExternalServiceException, InventoryException {
        PseudoWireCommandBuilder builder = new PseudoWireCommandBuilder(dbPw, userName);
    ...
    private long getPseudoWireID(NetworkPseudoWire pw) throws IOException, InventoryException, ExternalServiceException {
        if (pw.getVcID() > 0) {
            return pw.getVcID();
        }
        long id = pw.getAc1().getPseudoWireID();
        if (id > 0) {
            return id;
        }
        PseudowireLongIdPoolDto pool = InventoryConnector.getInstance().getPseudoWireLongIdPool(policy.getDefaultPseudoWirePoolName());
        for (PseudowireDto user : pool.getUsers()) {
            Long userID = user.getLongId();
            if (userID != null) {
                id = Math.max(id, userID.longValue());
            }
        }
        id++;
        return id;
