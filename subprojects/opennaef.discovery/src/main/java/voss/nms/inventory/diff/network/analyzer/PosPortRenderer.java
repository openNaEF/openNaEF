...
public class PosPortRenderer extends AbstractHardPortRenderer<POSImpl> {
    protected static final Logger log = LoggerFactory.getLogger(LogConstants.DIFF_SERVICE);

    public PosPortRenderer(POSImpl port, String parentID, int depth, Map<String, String> map) {
    ...
    protected String buildAbsoluteName(String parentAbsoluteName) {
        return AbsoluteNameFactory.getPosPortAbsoluteName(getParentAbsoluteName(), getModel());
    }
    ...
    public String getValue(Enum<?> attr_) {
        ...
        switch (attr) {
            ...
            case BANDWIDTH:
                Long band = port.getBandwidth();
                if (band == null) {
                    PortSpeedValue.Oper oper = port.getPortOperationalSpeed();
                    if (oper != null) {
                        band = oper.getValue();
                    }
                }
