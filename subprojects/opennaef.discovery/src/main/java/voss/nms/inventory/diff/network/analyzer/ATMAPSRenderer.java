...
public class ATMAPSRenderer extends AbstractHardPortRenderer<AtmAPSImpl> {
    protected static final Logger log = LoggerFactory.getLogger(LogConstants.DIFF_SERVICE);

    public ATMAPSRenderer(AtmAPSImpl aps, String parentID, int depth, Map<String, String> map) {
    ...
    protected String buildAbsoluteName(String parentAbsoluteName) {
        String ifName = AbsoluteNameFactory.getAtmApsName(getModel());
        return AbsoluteNameFactory.getAtmApsAbsoluteName(parentAbsoluteName, ifName);
    }
    ...
    public Map<String, String> getMemberPorts() {
        ...
        AtmAPSImpl aps = getModel();
        for (AtmPort member : aps.getMemberPort()) {
