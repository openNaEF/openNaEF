...
/**
 * TODO LinkCommandBuilderを使っての更新時にcontextコマンドの実行に失敗する
 * naef shell が link の absolute-name のパースに失敗するのが原因
 * <p>
 * voss.nms.inventory.builderConfig.LinkCommandBuilder のコピー
 */
public class _LinkCommandBuilder extends AbstractCommandBuilder {
     ...
     * Linkを新規作成する.
     */
    public _LinkCommandBuilder(PortDto port1, PortDto port2, String editorName) {
     ...
     * Linkを編集・削除する.
     *
     * @param link, editorName
     */
    public _LinkCommandBuilder(LinkDto link, String editorName) {
...
//            InventoryBuilder.changeContext(this.cmd, this.link);
            // XXX workaround
            WorkaroundUtil.changeContext(cmd, link);
    ...
    private void removeVlanLink(VlanSegmentDto link) {
        ...
        // vlan-link から lower-layer link を割り当て解除
        // XXX workaround InventoryBuilder.changeContext(cmd, link);
        WorkaroundUtil.changeContext(cmd, link);
