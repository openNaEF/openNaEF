...
public class VNicListCollector {
    ...
    private static void collectProperties(VimPortType methods, ServiceContent sContent) throws Exception {
        ...
        pSpecName.setPathSet(new String[]{"name", "config.hardware.device"});
        ...
        for (ObjectContent oc : objectContents) {
            for (DynamicProperty dp : oc.getPropSet()) {
                ...
                if (val instanceof ArrayOfVirtualDevice) {
                    VirtualDevice[] vDevices = ((ArrayOfVirtualDevice) val).getVirtualDevice();
                    for (VirtualDevice vDevice : vDevices) {
                        if (vDevice instanceof VirtualEthernetCard)
                            System.out.println(vDevice.getDeviceInfo().getLabel());
                    }
                }
            }
        }
