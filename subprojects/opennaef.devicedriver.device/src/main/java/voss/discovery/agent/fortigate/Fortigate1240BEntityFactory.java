...
public class Fortigate1240BEntityFactory extends AbstractFortigateEntityFactory {
    private static final Logger log = LoggerFactory.getLogger(Fortigate1240BEntityFactory.class);

    @Override
    public Port createPort(PhysicalEntry pe, Device device) {
        String ifName = pe.physicalName;
        int ifIndex = pe.ifindex;
        String numberPart = ifName.replace("port", "");
        int portIndex = 0;
        try {
            portIndex = Integer.parseInt(numberPart);
        } catch (Exception e) {
        }
    ...
    public void createVdom(DeviceAccess access, MplsVlanDevice device) throws IOException, AbortedException {
        ...
        try {
            List<StringSnmpEntry> entries = SnmpUtil.getStringSnmpEntries(access.getSnmpAccess(), FortigateMib.fnVdEntName);
            ...
            List<IntSnmpEntry> aliasPortEntries = SnmpUtil.getIntSnmpEntries(access.getSnmpAccess(), FortigateMib.fnIntfEntVdom);
