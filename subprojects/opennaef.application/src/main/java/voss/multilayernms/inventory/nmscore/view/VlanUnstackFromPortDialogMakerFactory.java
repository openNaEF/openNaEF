...
import voss.multilayernms.inventory.nmscore.view.portedit.VlanUnstackFromPortDialogMaker;

import java.io.IOException;
import java.util.List;

public class VlanUnstackFromPortDialogMakerFactory {

    private final static Logger log = LoggerFactory.getLogger(VlanUnstackFromPortDialogMakerFactory.class);

    public static VlanUnstackFromPortDialogMaker getDialogMaker(ObjectFilterQuery query) throws IOException, ExternalServiceException, InventoryException {
        ...
        if (port instanceof HardPortDto || port instanceof EthLagIfDto) {
            return new VlanUnstackFromPortDialogMaker(inventoryId, (PortDto) port);
        } else {
            throw new InventoryException("unexpected type: " + port.getAbsoluteName());
        }
    }
