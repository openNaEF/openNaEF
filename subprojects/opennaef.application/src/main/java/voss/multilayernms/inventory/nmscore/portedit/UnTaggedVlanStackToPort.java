...
public class UnTaggedVlanStackToPort implements IPortEditUpdate {
    ...
    public UnTaggedVlanStackToPort(PortDto port, PortEditModel model, String user) {
    ...
    public void update() throws RuntimeException, IOException, InventoryException, ExternalServiceException {
        try {
            if (this.model instanceof HardPort) {
                ...
                for (String vid : model.getVlanIds()) {
                    ...
                    List<String> portAbsoluteName = PortRenderer.getUnTaggedPortAbsoluteNames(vlanif);
                    List<String> portIfName = PortRenderer.getUnTaggedPortIfNames(vlanif);
                    ...
                    builder.setUntaggedPorts(portAbsoluteName, portIfName);
            ...
            } else if (this.model instanceof LagPort) {
                LagPort model = (LagPort) this.model;
                VlanIdPoolDto pool = VlanUtil.getPool(model.getVlanPoolName());
                if (model.getVlanId().isEmpty()) {
                    throw new InventoryException("VLAN ID is not set.");
                } else if (model.getTagMode() == null || model.getTagMode().isEmpty()) {
                    throw new InventoryException("TagMode is not selected.");
                } else if (!PortEditConstraints.isPortSetVlanIdEnabled(port)) {
