...
public class RootSubnetNamespaceEditPage extends WebPage {
    ...
    public RootSubnetNamespaceEditPage(WebPage backPage) {
        this(backPage, null, null);
    }

    public RootSubnetNamespaceEditPage(WebPage backPage, IpSubnetNamespaceDto subnet) {
        this(backPage, subnet, null);
    }

    public RootSubnetNamespaceEditPage(WebPage backPage, IpSubnetNamespaceDto subnet, IpSubnetNamespaceDto parent) {
        try {
            ...
            add(topLink);
            BookmarkablePageLink<Void> refresh = new BookmarkablePageLink<Void>("refresh", RootSubnetNamespaceEditPage.class);
            add(refresh);
    ...
    public void processCommit() throws Exception {
        RootIpSubnetNamespaceCommandBuilder builder;
        if (this.subnet == null) {
            builder = new RootIpSubnetNamespaceCommandBuilder(this.editorName);
        } else {
            builder = new RootIpSubnetNamespaceCommandBuilder(this.subnet, this.editorName);
        }
        builder.setVpnPrefix(this.vpnPrefix);
